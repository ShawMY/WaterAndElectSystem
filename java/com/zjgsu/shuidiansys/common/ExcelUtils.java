...
public class ExcelUtils {
     ...
     * @throws IOException
     */
    public static void exportExcel(String excelName, String sheetName, String[] headers, String[] ds_titles, int[] ds_format, int[] widths, List<Map<String,Object>> data , HttpServletRequest request, HttpServletResponse response) throws IOException {
...
//            }
    ...
    public static Map<String,Object> importExcel(String[] ds_titles, MultipartFile file){
        Map<String,Object> result = new HashMap<String, Object>();
        List<Map<String,Object>> mapList = new ArrayList<Map<String, Object>>();
        if(file == null){
            result.put("code","500");
            result.put("msg","file is null");
            result.put("date",mapList);
            return result;
        }
        String name = file.getOriginalFilename();
        long size = file.getSize();
        if(name == null || ReadExcel.EMPTY.equals(name) && size==0){
            result.put("code","500");
            result.put("msg","file name is null");
            result.put("date",mapList);
            return result;
        }
        //读取Excel数据到List中
        try {
            List<ArrayList<String>> list = new ReadExcel().readExcel(file);
            for(ArrayList<String> arr:list){
                Map<String,Object> map = new HashMap<String, Object>();
                for(int i =0;i<ds_titles.length;i++) {
                    System.out.println(ds_titles[i]+" : "+arr.get(i));
                    if(arr.get(i)==null){
                        map.put(ds_titles[i],"");
                    }else {
                        map.put(ds_titles[i], arr.get(i));
